...
The repository doubles as a composite action. The action tag and the CLI version move together: `@v0.4.6` runs residoo 0.4.6 from npm.
  ...
  scan:
    ...
    steps:
      ...
      - uses: actions/setup-node@v4
        with:
          node-version: "20"
      - run: npm install -g residoo@0.4.6 && residoo scan --project . --fail-on-find
```
