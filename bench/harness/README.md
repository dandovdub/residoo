...
- `detect-secrets`: installed via uv into `bench/tools/uv`. Scored in its documented offline mode (`scan --all-files -n`; its own help: "-n, --no-verify  Disables additional verification of secrets via network call"). Baseline JSON parsed from stdout; output is SHA-1 hashes only (redacted by design, matched at file+line). One finding per baseline entry; the tool dedupes same-file re-occurrences of a secret at the first line, which the distinct-value metric represents honestly. Claims generic file scanning (its README's `--all-files` non-git form), so it is scored on every class.
- `trufflehog`: pinned binary, `filesystem <fixture home> --json --no-verification --results=verified,unknown,unverified --no-update`. Scored in its documented offline mode (`--no-verification`); `--results` is passed explicitly so unverified findings are always printed even on versions with a verified-only default; `--no-update` is its documented update-check off-switch. JSONL stdout parsed. Claims every class (generic filesystem scanner).
- `betterleaks`: pinned binary, `dir <fixture home> --report-format json --no-banner --regex-engine stdlib` (the gitleaks-successor `dir` form). Not dual-mode: validation is opt-in via `--validation`, which is never passed. `--regex-engine stdlib` keeps its default engine's wasm compilation cache write out of the scanned fixture; finding-set parity between the engines was verified on the corpus first. Claims every class.
- `kingfisher`: pinned binary, `scan <fixture home> --no-validate --format jsonl --no-dedup --no-rule-cache --no-update-check`. Scored in its documented offline mode (`--no-validate`); `--no-dedup` is its documented every-occurrence reporting mode (site-recall fairness); `--no-rule-cache` keeps its compiled-rule cache write out of the scanned fixture (its default location is under HOME). Claims every class.
- `ggshield`: see the design decision below.

### Dual-mode tools (optional live verification)

Some scanners verify candidate secrets against provider APIs during the scan by default and document an offline flag (TruffleHog `--no-verification`, Kingfisher `--no-validate`, detect-secrets `-n`). The rule, applied identically to each:

- Recall is scored ONLY from the documented offline mode. Scoring recall in a mode that phones out would conflate the recall axis with the egress axis.
- Egress is observed in BOTH modes, each under the full monitor. The offline run's verdict must be none-observed for the tool's offline claim to stand; the default mode is executed against the same corpus by a companion adapter (id `<tool>-default-verification`, declaring `egressOnly: {forTool, modeLabel, citation}`), and its observed connection attempts are reported factually, with CONNECT destinations listed and a citation to the vendor's own documentation describing verification. No editorializing: what was observed, and what their docs say.
- Fake values only: every planted credential is a pattern-true fake no provider ever issued, so a default-mode verification attempt sends only fake values at worst, and the refuse-and-log proxy trap refuses the connections anyway, so no verification request can leave the machine. Both facts are stated so nobody can claim the benchmark transmitted secrets.
- The scorer attaches the companion's egress record to the tool's row as a second, clearly labeled mode line (`egressModes` in `scoreboard.json`); companion records are never scored for recall or precision and never appear as separate tools.
...
Rendering scales with the field: `scoreboard.md` keeps one detail section per tool (so per-tool tables never widen as tools are added) and opens with a compact cross-tool matrix, one short cell per tool per class, so an 8-plus-tool field stays readable. `scoreboard.json` keeps its shape as the field grows: `scores` stays one entry per scored tool, and dual-mode tools additionally carry `egressModes` (one labeled entry per mode); egress-only companion records never add entries to `scores`.
