...
Everything else is documented at the canonical repository: what it scans
(43 sources), what it detects (35 verified vendor patterns), the `--seal`
encrypted vault, the security model, and why it makes zero network calls.
Start here: **https://github.com/dandovdub/residoo**

This launcher also exists to hold the `residoo` name on PyPI against
impersonation, a real and current attack pattern for security tools that is
documented in the project's own SECURITY.md. If you found a "residoo"
anywhere other than this package, npm's `residoo`, or that GitHub repository, don't run it.
